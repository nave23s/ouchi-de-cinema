...
- **ハブページ生成基準**：作曲家・アーティスト・ジャンルを問わず「slug付き作品3本以上」を生成条件とする。1〜2本はスキップし、バックフィル進行後に `node scripts/generate-hubs.js` を再実行すれば自動更新される
- **フォームCSVインポート時の正規化**：公開年の「年」表記（例：「2021年」→ 2021）と人名の長音符抜け（例：「ブリュワ」→「ブリュワー」）をパース時に必ず修正してから登録する

## 将来の装備リスト（条件が揃ったら導入する）

以下の条件に該当する状況になったとき、Claudeから「装備リストの○○の出番です」と提案すること。

| 条件 | 導入するもの |
|---|---|
| 2つ目のサイトを本格運営し始めたら | git worktree 並列運用 |
| note記事の生成を完全自動化したくなったら | claude -p 非対話モード |
| 毎週決まった保守作業が3つ以上溜まったら | Routines（定期実行） |
| movies.json破損事故が1度でも起きたら | 書き込み保護hookを即作成 |
| レビュー品質チェックの工程を分離したくなったら | subagents |
